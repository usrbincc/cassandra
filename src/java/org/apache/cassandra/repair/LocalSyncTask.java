...
public class LocalSyncTask extends SyncTask implements StreamEventHandler
{
    ...
    protected void startSync(List<Range<Token>> differences)
    {
        ...
        String message;
        logger.info("[repair #{}] {}", desc.sessionId, message = String.format("Performing streaming repair of %d ranges with %s", differences.size(), dst));
    ...
    {
        String message;
        logger.info("[repair #{}] {}", desc.sessionId, message = String.format("Sync complete between %s and %s on %s", desc.sessionId, r1.endpoint, r2.endpoint, desc.columnFamily));
        Tracing.traceRepair(message);
        set(stat);
    }
